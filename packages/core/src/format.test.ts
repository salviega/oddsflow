...
describe('formatAmount', () => {
	it('groups thousands, shows two decimals and the unit', () => {
		expect(formatAmount(1000n * ONE)).toBe('1,000.00 sDAI')
		expect(formatAmount(1_234_567n * ONE + ONE / 2n)).toBe('1,234,567.50 sDAI')
	})

	it('shows zero as zero', () => {
		expect(formatAmount(0n)).toBe('0.00 sDAI')
	})

	it('shows a threshold instead of zeros for dust', () => {
		expect(formatAmount(1n)).toBe('< 0.01 sDAI')
		expect(formatAmount(ONE / 100n - 1n)).toBe('< 0.01 sDAI')
		expect(formatAmount(ONE / 100n)).toBe('0.01 sDAI')
	})

	it('rounds down, never showing more than there is', () => {
		expect(formatAmount((ONE * 1999n) / 1000n)).toBe('1.99 sDAI')
	})

	it('respects other decimals', () => {
		expect(formatAmount(1_500_000n, 6)).toBe('1.50 sDAI')
	})
