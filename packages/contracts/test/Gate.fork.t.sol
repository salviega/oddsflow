...
contract GateForkTest is ForkBase {
    ...
    function test_gate_fillsAtFixedPriceExactIn() public {
        ...
        (uint256 qIn, uint256 qOut,) = router.quote(order, address(yes), address(SUSDS), 100e18, data);
        assertEq(qIn, 100e18);
        assertEq(qOut, 20e18, "quote: 100 YES at 0.20 -> 20 sUSDS");

        (uint256 amountIn, uint256 amountOut) = mockTaker.swap(order, address(yes), address(SUSDS), 100e18, data);
    ...
    function test_gate_fillsAtFixedPriceExactOut() public {
        ...
        (uint256 qIn, uint256 qOut,) = router.quote(order, address(yes), address(SUSDS), 20e18, data);
        assertEq(qIn, 100e18, "quote: 20 sUSDS out needs 100 YES in");
        assertEq(qOut, 20e18);
        mockTaker.swap(order, address(yes), address(SUSDS), 20e18, data);
