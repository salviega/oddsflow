// Which chain the web talks to: Gnosis Chain, or the local fork from
...
export const gnosisFork = defineChain({
	id: FORK_CHAIN_ID,
	name: 'Gnosis (local fork)',
	nativeCurrency: gnosis.nativeCurrency,
	rpcUrls: { default: { http: ['http://127.0.0.1:8545'] } },
	contracts: gnosis.contracts,
})

export const chain = isFork ? gnosisFork : gnosis

// The server reads through GNOSIS_RPC_URL, a key that never reaches the
// browser; the browser uses NEXT_PUBLIC_GNOSIS_RPC_URL (or Gnosis' public RPC).
const onServer = typeof window === 'undefined'
export const rpcUrl = isFork
	? 'http://127.0.0.1:8545'
	: onServer
		? (process.env.GNOSIS_RPC_URL ?? process.env.NEXT_PUBLIC_GNOSIS_RPC_URL)
...
/** Markets shown first (comma-separated), then the rest. */
...
export function explorerTx(hash: string): string | undefined {
	return isFork ? undefined : `https://gnosisscan.io/tx/${hash}`
}
